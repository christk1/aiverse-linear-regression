...
combined_feat = pd.DataFrame(data['land'].values / (data['X4 number of convenience stores'].values + 1), columns=['land_cov_stores'])
data = pd.concat([combined_feat, data], axis=1)

data = data.drop(columns=['X5 latitude', 'X6 longitude', 'X4 number of convenience stores'])
