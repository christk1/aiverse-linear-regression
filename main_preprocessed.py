...
tmp = data['X1 transaction date'].sort_values().index
tmp = data['Y house price of unit area'].reindex(tmp)
price1 = tmp.iloc[1:]
price_m1 = tmp.iloc[:-1]
print(np.corrcoef(price_m1, price1)[0, 1])

corr = data.corr()
# saleprice correlation matrix
plt.gcf().subplots_adjust(bottom=0.4, left=0.4)
cols = data.columns
cm = np.corrcoef(data[cols].values.T)
hm = sns.heatmap(cm, cbar=True, annot=True, square=True,
                 yticklabels=cols.values, xticklabels=cols.values)
plt.show()
data = data.drop(columns=['X6 longitude'])
# plot scatters
...
data = data.drop(data['X5 latitude'].idxmax())
...
data.loc[:, 'X5 latitude'] = scaler.fit_transform(data.loc[:, 'X5 latitude'].to_numpy().reshape(-1, 1))
